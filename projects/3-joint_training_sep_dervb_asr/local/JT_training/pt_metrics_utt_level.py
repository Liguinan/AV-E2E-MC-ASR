...
def run(wavscp, pkl, metric_res_dir, using_clean):
    ...
    for wav_file, est_wav_path in wavscp_dct.items():
        ...
        wav_file_lst.append(wav_file)
    assert len(sisnr_lst) == len(pesq_lst) == len(stoi_lst) == len(srmr_lst) == len(wav_file_lst)
    ...
    if using_clean=='true':
        ...
        # import pdb; pdb.set_trace()
        with open(f"{metric_res_dir}/metrics_clean_utt_level.log", 'w') as fw:
            for wav_file, sisnr, pesq, stoi, srmr in zip(wav_file_lst, sisnr_lst, pesq_lst, stoi_lst, srmr_lst):
                fw.write(f"{wav_file} sisnr:{round(sisnr, 2)} pesq:{round(pesq, 2)} stoi:{round(stoi, 4)}, srmr:{round(srmr, 2)}\n")
    else:
