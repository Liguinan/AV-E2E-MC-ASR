...
class TasnetDataset(Dataset):
    def __init__(self, annotations_file, mode, max_nspk = 3):
        ...
        self.new_wav_scp = params.new_wav_scp
        self.new_wav_scp_dct = {} # e.g. sep output
        if self.new_wav_scp: 
            new_wav_scp_path = None
            if mode == 'train_pretrain':
                new_wav_scp_path = params.training_wavscp
            if mode == 'val':
                new_wav_scp_path = params.validation_wavscp
            with open(new_wav_scp_path, 'r', encoding='utf-8') as fr:
                    lines = fr.readlines()
                    for line in lines:
                        line = line.strip()
                        line_lst = line.split()
                        self.new_wav_scp_dct[line_lst[0]] = line_lst[1]
        print(f"mode={mode}, {len(self.info)} utterance in pickle, new_wav_scp flag is: {self.new_wav_scp }, count {len(self.new_wav_scp_dct)} in new wav scp")
        
        self.mc_ref_wav_scp = params.mc_ref_wav_scp
        self.mc_ref_wav_scp_dct = {}
        if self.mc_ref_wav_scp:
    ...
    def __getitem__(self, idx):
        ...
        if self.new_wav_scp: 
            new_wav, _ = librosa.load(self.new_wav_scp_dct[wav_file], sr=params.sampling_rate, mono=False)
