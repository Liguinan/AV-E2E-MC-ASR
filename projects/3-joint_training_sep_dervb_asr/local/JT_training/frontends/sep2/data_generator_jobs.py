...
class CHDataset(object):
    def __init__(self, stage, inference_job_num, job):
        ...
        ### new mixture wav
        self.test_wav_scp_dct = {}
        with open(new_wav_scp_path, 'r') as fr:
            lines = fr.readlines()
            for line in lines:
                line = line.strip().split()
                self.test_wav_scp_dct[line[0]] = line[1]
        logger.info(f"all_new_mixture_wav_num: {len(self.test_wav_scp_dct)}")
    ...
    def get_data(self, wav_file):
        ...
        new_mixture_wav, _ = librosa.load(self.test_wav_scp_dct[wav_file], sr=sampling_rate, mono=False)
        # mix_wav = wav[:n_mic]
        # mix_wav = new_mixture_wav[:n_mic] / 32768
        mix_wav = new_mixture_wav[:n_mic]
