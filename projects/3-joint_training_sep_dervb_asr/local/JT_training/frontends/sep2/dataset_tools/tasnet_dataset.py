...
class TasnetDataset(Dataset):
    def __init__(self, annotations_file, new_mixture_wav_scp, max_nspk = 3):
        with open(annotations_file, 'rb') as fp:
            self.info = pickle.load(fp, encoding='utf-8')
        self.new_mixture_wav_scp_dct = {}
        with open(new_mixture_wav_scp, 'r') as fr:
            lines = fr.readlines()
            for line in lines:
                line = line.strip().split()
                self.new_mixture_wav_scp_dct[line[0]] = line[1]
        self.wav_list = list(self.info.keys())
        self.max_nspk = max_nspk
        print(f"new mixture wav scp num: {len(self.new_mixture_wav_scp_dct)}")
    ...
    def __getitem__(self, idx):
        ...
        ###new mixture wav load##########
        new_mixture_wav, _ = librosa.load(self.new_mixture_wav_scp_dct[wav_file], sr=params.sampling_rate, mono=False)
        mix_wav = new_mixture_wav[:, tgt_beg_idx:tgt_end_idx]
