...
class TasnetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return np.array(mix_wav).astype(np.float32), np.array([tgt_reverb_wav]).astype(np.float32), directions, nspk_in_wav, lip_video, mix_wav.shape[1], lip_video.shape[0], wav_file