...
#     torch_snapshot,
# )
from espnet.JT_training.asr_utils import (
    CompareValueTrigger,
    adadelta_eps_decay,
    add_results_to_json,
    format_mulenc_args,
...
# )
from espnet.JT_training.asr_init import (
    freeze_modules,
    load_trained_model,
    load_trained_modules,
)
...
from espnet.JT_training.io_utils import LoadInputsAndTargets

# from espnet.utils.training.batchfy import make_batchset
from espnet.JT_training.batchfy import make_batchset
...
class CustomUpdater(StandardUpdater):
    ...
    def update_core(self):
        ...
        if self.grad_noise:
            from espnet.JT_training.asr_utils import add_gradient_noise
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        if len(batch[0]) == 8:
            axs, ref_wav, wav_len, ilens, spk_doa, n_spk, vxs, ys = batch[0]

        # perform subsampling
        if self.subsampling_factor > 1:
            axs = [x[:: self.subsampling_factor, :] for x in axs]
        ...
        if axs[0].dtype.kind == "c":
            ...
            ref_wav_pad = pad_list([torch.from_numpy(x).float() for x in ref_wav], 0).to(
                device, dtype=self.dtype
            )
            wav_len = torch.Tensor(np.array(wav_len)).int().to(device)
            spk_doa = torch.Tensor(np.array(spk_doa).astype(np.float32)).to(device)
            n_spk = torch.Tensor(np.array(n_spk).astype(np.float32)).to(device)
        
        axs_all_pad = (axs_pad, ref_wav_pad, wav_len, spk_doa, n_spk)
        ...
        return axs_all_pad, vxs_pad, ilens, ys_pad
...
def load_pretrained_modules_mvdr(model_path, target_model, freeze_parms=False, map_location=None):
    # load trained model and obtained model dict
    if map_location is None:
        src_model_dict = torch.load(model_path, map_location=lambda storage, loc: storage)["model_state_dict"]
    else:
        src_model_dict = torch.load(model_path, map_location=map_location)["model_state_dict"]
    
    # obtained now need to trained model dict (namely, target model)
    tgt_model_dict = target_model.state_dict()
    
    # filter trained params which are needed by target model (from trained model dict, namely src_model_dict) 
    pretrained_dict = OrderedDict()
    for k, v in src_model_dict.items():
        if f"mvdr_net.{k}" in tgt_model_dict:
            # mvdr_net is customized by target model (here in e2e_asr_transfomer.py)
            pretrained_dict[f"mvdr_net.{k}"] = v
    
    # update target model params by pretrained dict
    tgt_model_dict.update(pretrained_dict)
    # load the updated dict to target model
...
def load_pretrained_modules_wpe(model_path, target_model, freeze_parms=False, map_location=None):
    # load trained model and obtained model dict
    if map_location is None:
        src_model_dict = torch.load(model_path, map_location=lambda storage, loc: storage)["model_state_dict"]
    else:
    ...
    pretrained_dict = OrderedDict()
    for k, v in src_model_dict.items():
        if f"dwpe_net.{k}" in tgt_model_dict:
            # mvdr_net is customized by target model (here in e2e_asr_transfomer.py)
            pretrained_dict[f"dwpe_net.{k}"] = v
...
def load_pretrained_modules_asr(model_path, target_model, match_keys, freeze_parms=False, map_location=None):
    # load trained model and obtained model dict
    if map_location is None:
...
def train(args):
    ...
    # load pretrained frontend model and backend asr model
    # mvdr trianed model
    if args.init_frontend_mvdr:
        load_pretrained_modules_mvdr(
            args.init_frontend_mvdr, 
            model, 
            freeze_parms=args.mvdr_frozen,
            map_location=f"cuda:{torch.cuda.current_device()}" if args.ngpu > 0 else "cpu",
        )
        logging.info("Loading pretrained model " + args.init_frontend_mvdr)
        logging.info("mvdr model frozen status: " + str(args.mvdr_frozen))
    
    # wpe trianed model
    if args.init_frontend_wpe:
        load_pretrained_modules_wpe(
            args.init_frontend_wpe, 
            model, 
            freeze_parms=args.wpe_frozen,
            map_location=f"cuda:{torch.cuda.current_device()}" if args.ngpu > 0 else "cpu",
        )
        logging.info("Loading pretrained model " + args.init_frontend_wpe)
        logging.info("wpe model frozen status: " + str(args.wpe_frozen))
        
    # asr trained model
    if args.init_asr:
        match_keys = r'(encoder|decoder|ctc)'
        load_pretrained_modules_asr(
            args.init_asr, model, match_keys, freeze_parms=args.asr_frozen,
            map_location=f"cuda:{torch.cuda.current_device()}" if args.ngpu > 0 else "cpu",
        )
        logging.info("Loading pretrained model " + args.init_asr)
        logging.info("asr model frozen stautus " + str(args.asr_frozen))
    ...
    load_tr = LoadInputsAndTargets(
        ...
        use_clean_ref=args.use_clean_ref
    )
...
def recog(args):
    ...
    load_inputs_and_targets = LoadInputsAndTargets(
        ...
        use_clean_ref=train_args.use_clean_ref
    )
    ...
    if args.batchsize == 0:
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                # )
                # ['axs', 'ref_wav', 'wav_len', 'ilens', 'spk_doa', 'n_spk', 'vinput1']
                afeat = (
                    feat[0][0], feat[1][0], feat[2][0], feat[4][0], feat[5][0]
                )
                # import pdb; pdb.set_trace()
                vfeat = (
                    feat[6][0]
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:
                    ...
                    # import pdb; pdb.set_trace()
                    nbest_hyps = model.recognize(
                        afeat, vfeat, args, train_args.char_list, rnnlm, uttid=name
                    )
