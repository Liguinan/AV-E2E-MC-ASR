...
def main(argv):
    ...
    early_rvb_tgt_wav_scp_path = argv[2]
    rvb_only_tgt_wav_scp_path = argv[3]
    input_wav_scp_path = argv[4]
    output_wav_path = argv[5]

    if not os.path.exists(output_wav_path):
        os.makedirs(output_wav_path)

    clean_wav_scp_dict = wav_scp_2_dict(clean_wav_scp_path)
    early_rvb_tgt_wav_scp_dict = wav_scp_2_dict(early_rvb_tgt_wav_scp_path)
    rvb_only_tgt_wav_scp_dict = wav_scp_2_dict(rvb_only_tgt_wav_scp_path)
    ...
    for utter_id, wav_path in input_wav_scp_dict.items():
        ...
        y_rvb_only, sr_rvb_only = librosa.load(rvb_only_tgt_wav_scp_dict[utter_id], mono=False, sr=None)
        y_input, sr_input = librosa.load(input_wav_scp_dict[utter_id], mono=False, sr=None)
        clean_utter_id = utter_id.split("-", 1)[1]
        y_clean, sr_clean = librosa.load(clean_wav_scp_dict[clean_utter_id], mono=False, sr=None)
        
        assert y_input.shape[0] == 15
        assert y_input.shape == y_rvb_early.shape
        assert y_input.shape == y_rvb_only.shape
        assert y_input.shape[1] == y_clean.shape[0]

        wav_data_mc = np.row_stack((y_input, y_clean, y_rvb_early[0], y_rvb_only[0]))
        ...
        soundfile.write(wav_file, np.transpose(wav_data_mc), sr_input)
