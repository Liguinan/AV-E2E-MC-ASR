...
from espnet.JT_training.conv_stft_no_norm_K_pad_zeros import STFT
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, aidim, vidim, odim, args, ignore_id=-1):
        ...
        if getattr(args, "use_frontend", False):
            if getattr(args, "use_beamformer", False):
                if getattr(args, "beamformer_type", False) == 'mvdr':
                    from espnet.JT_training.frontends.sep.pt_avnet_mvdr import ConvTasNet
                    import espnet.JT_training.frontends.sep.params as mvdr_params
    ...
    def forward(self, axs_pad_all, vxs_pad, ilens, ys_pad):
        ...
        if self.use_frontend:
            if self.use_beamformer:
                axs_pad_to_mvdr = axs_pad_wav.permute(0, 2, 1).contiguous()
                ...
                if self.beamformer_type == 'mvdr':
                    ests, _ = self.mvdr_net([axs_pad_to_mvdr, spk_doa, n_spk.squeeze(), vxs_pad, wav_len // 256 - 1]) 
                else:
                ...
                norm_mix = torch.max(torch.abs(axs_pad_to_mvdr[:,0,:]), dim=1, keepdim=True)[0]
                ests_scale = ests_wav * norm_mix / (norm_ests + 1e-10)
        
        
        # [0.0] transform wav to fbank feature 
        # -1. WAV -> STFT
        B, C, t = ests_scale.unsqueeze(1).shape
        # (B, C, t) -> (BxC, t)
    ...
    def encode(self, afeat_all, vfeat):
        ...
        wav_len = torch.as_tensor(wav_len).unsqueeze(0)
        vfeat = torch.as_tensor(vfeat).unsqueeze(0)
        
        # front-end model forward
        if self.use_frontend:
            if self.use_beamformer:
                afeat_to_mvdr = afeat_wav.permute(0, 2, 1).contiguous()
                # x.shape: (B, C, T); directions.shape: (B, 3); spk_num: (16); lip_video: (B, T, 512); seq_len: None
                # FFT=512, HOP_SIZE=256
                if self.beamformer_type == 'mvdr':
                    ests, _ = self.mvdr_net([afeat_to_mvdr, spk_doa, n_spk.squeeze(), vfeat, wav_len // 256 - 1]) 
                else:
                    raise RuntimeError(f"not support self.beamformer:{self.beamformer_type}")
                # loss calculate
                ests_wav = ests[0]
                ...
                # scale wav mag by mixture since sisnr output is a projection
                norm_ests = torch.max(torch.abs(ests_wav), dim=1, keepdim=True)[0]
                norm_mix = torch.max(torch.abs(afeat_to_mvdr[:,0,:]), dim=1, keepdim=True)[0]
                ests_scale = ests_wav * norm_mix / (norm_ests + 1e-10)
        ...
        return enc_output.squeeze(0), ests_scale
