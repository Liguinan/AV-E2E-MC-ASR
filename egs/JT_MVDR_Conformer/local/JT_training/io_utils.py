...
class LoadInputsAndTargets(object):
    ...
    def __init__(
        ...
        ## customized 
        # 15 channel mixture speech
        self.CHN_NUM = 15  # hard code
        self.use_clean_ref = use_clean_ref
        ...
        for uttid, info in batch:
            ...
            if self.load_input:
                ...
                for idx, inp in enumerate(info["input"]):
                    ...
                    # [1] audio input
                    if inp["afeat"] != "":  # for train_pretrain and val
                        _, ax_all = self._get_from_loader(
                            filepath=inp["afeat"], filetype=inp.get("filetype", "mat")
                        )
                        ax_all = ax_all / 2 ** 15
                        # ax_all_2 = self._get_from_loader(
                        #     filepath=inp["afeat_wav"], filetype=inp.get("filetype", "sound")
                        # )
                        # ax_all_3 = self._get_from_loader(
                        #     filepath=inp["afeat_wav"], filetype=inp.get("filetype", "wav")
                        # )
                    else: # for test and replay
                        ax_all = self._get_from_loader(
                            filepath=inp["afeat_wav"], filetype=inp.get("filetype", "sound")
                        )
                        
                    # idx: 0-14 -> mixture, 15 -> clean wav, 16 -> rvb clean wav
                    ax = ax_all[:, :self.CHN_NUM]
                    ax_feats_dict.setdefault(inp["name"], []).append(ax)
                    if self.use_clean_ref: 
                        ax_ref = ax_all[:, self.CHN_NUM]  # clean wav
                    else:
                        ax_ref = ax_all[:, self.CHN_NUM + 1] # rvb clean wav for mvdr only
                    ax_feats_dict.setdefault("ref_wav", []).append(ax_ref)
                    
                    # wav len for front-end mse calculation
                    ax_feats_dict.setdefault("wav_len", []).append(np.array([len(ax_ref)]))
                    
                    # [2] audio shape for asr use (25ms window, 10ms hopsize)
                    ax_feats_dict.setdefault("ilens", []).append(inp['ashape'][0])
                    
                    # [3] other audio info
                    nspk_in_wav = inp['n_spk']
                    spk_doa = inp['spk_doa']
                    time_idx = inp['time_idx']
                    ax_feats_dict.setdefault('spk_doa', []).append(np.array(spk_doa).astype(np.float32))
                    ax_feats_dict.setdefault('n_spk', []).append(np.array([nspk_in_wav]).astype(np.float32))
